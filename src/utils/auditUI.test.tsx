...
function dashboard(month: string) {
 const summary = calculateMonthSummary(month, [tx], [], budget, [], now, 10);
...
describe('audit UI rendering', () => {
 ...
 it('opens history on the spending month, including spending before payday', () => {
  const html=renderToStaticMarkup(<FeedbackProvider><HistoryView transactions={[tx]} categories={[]} bankAccounts={[]} paymentCards={[]} period={getAccountingPeriod('2026-09',10,now)} initialView="spending" onDeleteTransaction={noop} onUpdateTransaction={noop}/></FeedbackProvider>);
