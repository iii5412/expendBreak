...
export function spendingConclusion(summary: MonthSummary): string {
  if (summary.spendPeriodStatus === 'closed') {
    return `마감된 소비 월입니다. 생활비 ${formatKRW(summary.confirmedVariableExpenses)} 사용 · ${summary.remainingAllowance < 0 ? `한도 ${formatKRW(-summary.remainingAllowance)} 초과` : `한도 잔여 ${formatKRW(summary.remainingAllowance)}`}`;
  }
  if (summary.spendPeriodStatus === 'upcoming') return '아직 시작하지 않은 소비 월입니다. 예정 수입과 고정 지출을 확인해 주세요.';
  ...
  return '현재 속도면 소비 월말까지 생활비가 남습니다.';
}
