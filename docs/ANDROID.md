...
Android 앱의 `설정 & AI`에서 **문자에서 지출 찾기**를 켜고 SMS 권한을 허용하면 실시간 수신과 앱 실행 시 문자함 확인을 함께 사용한다. 앱이 꺼져 있을 때 도착한 결제 문자도 다음 실행·복귀 때 찾는다. 분석 결과는 바로 지출에 넣지 않고 확인 대기 후보로 보여 주며, 사용자가 `등록 승인`을 눌렀을 때만 거래로 저장한다.

- `RECEIVE_SMS`는 새 문자를 빠르게 받는 데, `READ_SMS`는 앱 실행 시 놓친 문자를 복구하는 데 사용한다. 알림 접근 권한은 사용하지 않는다.
- 기존 사용자는 변경된 읽기 범위에 한 번 동의해야 문자함 확인이 시작된다. 최초 기준은 기능 도입 APK의 기기 설치 시각이며 이후 앱 업데이트에서도 유지된다.
- 금액과 승인·취소 표현이 함께 있는 문자만 앱 전용 후보 큐에 넣는다. 일반 문자와 카드사 앱 푸시는 거래 후보로 저장하지 않는다.
- 문자 원문과 발신번호는 서버·Firestore·AI에 전송하지 않는다. 승인하거나 후보에서 제외하면 기기 후보 큐에서 원문을 제거하고 처리 키만 남긴다.
- 승인번호가 있으면 승인번호를 사용하며, 없는 문자는 메시지 ID를 포함한 지문으로 실제 동일 금액 연속 결제가 합쳐지지 않게 한다.
- 카드가 여러 장이면 계좌·카드 화면에서 각 카드의 끝 4자리를 등록해야 정확히 연결된다. 전체 카드번호는 저장하지 않는다.
- 기능을 끄면 해당 계정의 처리 대기 문자를 즉시 삭제한다.
- 설정에서 확인 시작 시각, 마지막 성공 시각, 최근 검사·후보 건수와 `지금 확인`을 제공한다.

Google Play 배포 시 SMS 기반 자금 관리 사용 사례로 민감 권한 선언과 심사를 별도로 완료해야 한다. 직접 배포 APK에서도 최초 활성화 시 Android 런타임 권한 동의가 필요하다.
