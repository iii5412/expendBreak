...
Android 앱의 `설정 & AI`에서 **SMS 카드 지출 후보 받기**를 켜고 SMS 권한을 허용하면, 기능을 켠 뒤 새로 도착하는 SMS만 처리한다. 분석 결과는 바로 지출에 넣지 않고 앱의 확인 대기 후보로 보여 주며, 사용자가 `등록 승인`을 눌렀을 때만 거래로 저장한다.

- `RECEIVE_SMS`만 요청하며 기존 문자함을 읽는 `READ_SMS`와 알림 접근 권한은 사용하지 않는다.
- 금액과 승인·취소 표현이 함께 있는 문자만 앱 전용 임시 큐에 넣는다. 일반 문자와 카드사 앱 푸시는 처리하지 않는다.
- 문자 원문은 서버·Firestore·AI에 전송하지 않는다. 승인하거나 후보에서 제외할 때 기기 임시 큐의 원문을 제거한다.
- 승인번호가 있으면 승인번호, 없으면 카드·금액·사용처·시각 지문으로 중복 등록을 막는다.
